...
// Skip fetching positions if still paused
if (wallet.paused) return;
  ...
  if (!positions.length) {
    // Update last_checked even if no positions
  ...
  // 7️⃣ Only fetch unresolved picks if wallet is not paused
  if (!wallet.paused) {
    ...
    livePicksCount = livePicksRows.length;
